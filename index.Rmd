...
**THERE ARE 2450 TOTAL ADJUSTMENTS**
```{r}
...
**95.4 PERCENT OF LOTS HAVE HAD AN ADJUSTMENT MADE**
```{r}
...
**THESE 10 STYLES HAD THEIR ENTIRE INVENTORY(INITIAL PCS) USED**
```{r, echo=FALSE}
...
**THESE 10 STYLES HAD THE MOST INVENTORY(INITIAL PCS) USED**
```{r, echo=FALSE}
every_style = unique(theoretical_lot_audits$Style.[which(theoretical_lot_audits$Initial.Stock.Pcs != "0")])
...
style_used_df = data.frame(style_id = every_style, amount_used = every_style_used_pcs, initial_amount = every_style_sum_initial)

top_10_used_styles = style_used_df[order(style_used_df$amount_used, decreasing = TRUE), ][1:10, c(1,2)]
top_10_used_styles
```
```{r}
ggplot(top_10_used_styles, aes(x = style_id, y = amount_used)) +
  geom_col(fill = "steelblue") + 
  labs(title = "10 Most Used Style Ids", x = "Style Id", y = "Amount Used")
...
**LOTS THAT RECEIVED INVENTORY PER YEAR**
...
ggplot(lots_per_year_df, aes(x = year, y = lots_received)) +
  geom_col(fill = "steelblue") + 
  labs(title = "Lots That Received Inventory Per Year", x = "Year", y = "Number of Lots")
